...
// This provides real-time synchronization of editable data across all users
...
class RealTimeCMS {
  constructor() {
    ...
    this.githubConfig = {
      owner: 'MattVerstappen',
      repo: 'mattverstappen.github.io',
      branch: 'main'
    };
    this.initFirebase();
  }
  ...
  async setupFirebaseAuth() {
    ...
    // Firebase configuration
      const firebaseConfig = {
          apiKey: "AIzaSyBuAHxBJe_otANnRtO1In72-AgMMdmuP_c",
          authDomain: "matthewderekrall-8f18b.firebaseapp.com",
          projectId: "matthewderekrall-8f18b",
          storageBucket: "matthewderekrall-8f18b.firebasestorage.app",
          messagingSenderId: "588951170096",
          appId: "1:588951170096:web:792514688787f4754bd1f5",
          measurementId: "G-YF586F1B7E"
      };
  ...
  setupContentSync() {
    // Setup real-time data sync for ALL users (not just admins)
    if (this.db) {
    ...
    // Always load initial data for all users
  ...
  setupDemoAPI() {
    ...
    this.channel = new BroadcastChannel('sms-data-sync');
    this.channel.addEventListener('message', (event) => {
      if (event.data.type === 'data-update' && event.data.page === this.currentPage) {
      ...
      if (event.data.type === 'data-reset' && event.data.page === this.currentPage) {
        // Reload page data when admin resets
        location.reload();
      }
    });
  }
  ...
  hideAuthenticatedUI() {
    ...
    // Hide CMS toolbar
    const toolbar = document.getElementById('cms-toolbar');
    const githubToolbar = document.getElementById('github-toolbar');
    if (toolbar) {
      toolbar.classList.remove('active');
      toolbar.style.display = 'none';
    }
    if (githubToolbar) {
      githubToolbar.style.display = 'none';
    }
  ...
  toggleCMSMode() {
    ...
    if (this.cmsMode) {
      ...
      // Store original data and enable editing
  ...
  async getGitHubFileContent(filename) {
    try {
      ...
      console.error('Failed to get GitHub file data:', error);
      return null;
    }
  }
  ...
  applyChangesToHTML(htmlContent, changes) {
    ...
    // Apply each change to the HTML data
    Object.keys(changes).forEach(field => {
      const fieldValue = changes[field];

      // Find and replace data in data-field attributes
...
Changes made through CMS interface`;
  ...
  resetPage() {
    if (confirm('Reset all changes to original data? This cannot be undone.')) {
      ...
      this.showSuccessMessage('Page data has been reset to original state.');
    }
  }
